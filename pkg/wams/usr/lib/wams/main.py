...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if link.startswith("whatsapp://"):
            new_url = link.replace("whatsapp://", "https://web.whatsapp.com/")
            current_webview = window.tabs.currentWidget()
            if current_webview:
                current_webview.load(QUrl(new_url))
        elif "api.whatsapp.com/send" in link:
            new_url = link.replace("https://api.whatsapp.com/", "https://web.whatsapp.com/")
